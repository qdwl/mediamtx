...
// #cgo windows,amd64 LDFLAGS: -L${SRCDIR}/../../thirdparty/ffmpeg-n6.1.2-win-amd64/lib -lavcodec -lavutil -lswscale -lswresample
// #cgo windows,amd64 CFLAGS: -I${SRCDIR}/../../thirdparty/ffmpeg-n6.1.2-win-amd64/include
...
func (t *AudioTranscoder) initDecoder(forma format.Format) error {
	...
	t.decCtx.channel_layout = C.uint64_t(C.av_get_default_channel_layout(t.decCtx.channels))
...
func (t *AudioTranscoder) initEncoder(forma format.Format) error {
	...
	t.encCtx.channel_layout = C.uint64_t(C.av_get_default_channel_layout(t.encCtx.channels))
	t.encCtx.bit_rate = C.int64_t(32000)
...
func (t *AudioTranscoder) initSwr(decCtx *C.AVCodecContext, encCtx *C.AVCodecContext) error {
	t.swrCtx = C.swr_alloc_set_opts(nil,
		C.int64_t(encCtx.channel_layout),
		int32(encCtx.sample_fmt),
		C.int(encCtx.sample_rate),
		C.int64_t(decCtx.channel_layout),
	...
	for {
		...
		if t.decCtx.channel_layout == 0 {
			if t.decFrame.channel_layout != 0 {
				...
				t.decCtx.channel_layout = C.uint64_t(C.av_get_default_channel_layout(t.decFrame.channels))
			}
		}
...
func (t *AudioTranscoder) encode() ([]AudioPacket, error) {
	...
	for {
		...
		t.encFrame.pts = (C.int64_t(t.nextOutPts) + C.av_rescale(C.int64_t(t.encCtx.frame_size)*C.int64_t(frameCount), 1000*1000, C.int64_t(t.encCtx.sample_rate))) / 1000
		frameCount++
	...
	t.nextOutPts += int64(C.av_rescale(C.int64_t(t.encCtx.frame_size)*C.int64_t(frameCount), 1000*1000, C.int64_t(t.encCtx.sample_rate)))
